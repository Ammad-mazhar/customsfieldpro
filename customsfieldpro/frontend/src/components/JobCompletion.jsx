...
// ── Step indicator ────────────────────────────────────────────────────────────
const STEP_LABELS = ['Checklist', 'Photos', 'Signature', 'Summary']
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ textAlign: 'center', marginBottom: 4 }}>
        ...
        <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Upload photos to document the job. At least 1 after photo is required.</p>
      </div>
      ...
      {attempted && !hasAfter && (
        <div style={{ padding: '10px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, fontSize: 12.5, color: '#dc2626', fontWeight: 500 }}>
          ⚠ At least 1 after photo is required before proceeding.
        </div>
      )}
...
function Step3({ job, data, onChange, onNext, onBack, attempted }) {
  ...
  const [empty, setEmpty] = useState(!data.signatureDataUrl)

  useEffect(() => {
    if (data.signatureDataUrl && sigRef.current) {
      sigRef.current.fromDataURL(data.signatureDataUrl)
      setEmpty(false)
    }
  }, [])
  ...
  function handleClear() {
    ...
    onChange({ ...data, signatureDataUrl: '' })
    setEmpty(true)
  }
  ...
  function handleEnd() {
    if (sigRef.current && !sigRef.current.isEmpty()) {
      ...
      onChange({ ...data, signatureDataUrl: url })
      setEmpty(false)
    }
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 12, color: '#9ca3af', fontWeight: 400, marginLeft: 6 }}>sign with finger or mouse</span>
        </label>
        <div style={{
          position: 'relative',
          border: `2px solid ${attempted && !sigOk ? '#dc2626' : empty ? '#2563eb' : '#16a34a'}`,
          ...
          <SignatureCanvas
            ...
            canvasProps={{ width: 580, height: 200, style: { display: 'block', width: '100%', height: 200, cursor: 'crosshair' } }}
            onEnd={handleEnd}
          />
          {empty && (
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', paddingTop: 4 }}>
        ...
        <button onClick={handleNext} style={{ padding: '9px 22px', borderRadius: 8, border: 'none', background: '#2563eb', color: '#fff', fontSize: 13.5, fontWeight: 600, cursor: 'pointer' }}>
          Review Summary →
        </button>
      </div>
    </div>
...
function Step4({ job, data, onConfirm, onBack, confirming, success }) {
  const checklist  = getChecklist(job.type)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ border: '1px solid #e8e9ec', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ padding: '10px 16px', background: '#f9fafb', borderBottom: '1px solid #e8e9ec' }}>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: '#374151' }}>Client Signature — {data.clientPrintedName}</span>
        </div>
        <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
          {data.signatureDataUrl && (
            <img src={data.signatureDataUrl} alt="Signature" style={{ height: 52, maxWidth: 160, border: '1px solid #e8e9ec', borderRadius: 6, background: '#fff', objectFit: 'contain', padding: 4 }} />
          )}
          <span style={{ fontSize: 12.5, color: '#16a34a', fontWeight: 500 }}>✓ Client confirmed</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', paddingTop: 4 }}>
        ...
        <button onClick={onConfirm} disabled={confirming}
          style={{ padding: '10px 26px', borderRadius: 8, border: 'none', background: confirming ? '#86efac' : '#16a34a', color: '#fff', fontSize: 14, fontWeight: 700, cursor: confirming ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: 8, transition: 'background 0.2s' }}>
          {confirming ? (
            <>
...
export default function JobCompletion({ job, onClose, onComplete }) {
  ...
  // Step 2 validation
  function tryStep2() {
    if (data.afterPhotos.length === 0) { setAttempted(true); return }
    setAttempted(false)
    setStep(3)
  }
    ...
    >
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: success ? '40px 32px' : '24px' }}>
          {!success && <StepIndicator step={step} total={4} />}
          ...
          {step === 4 && (
            <Step4
              ...
              onBack={() => { setAttempted(false); setStep(3) }}
