...
import { getJobs as storeGetJobs, getInvoices as storeGetInvoices, getQuotes as storeGetQuotes, getRequests as storeGetRequests, getParts, getLowStockCount } from '../data/store'
...
export default function Dashboard() {
  ...
  const { data: invoices } = useApiData(invoicesApi.getInvoices,  storeGetInvoices)
  const { data: quotes }   = useApiData(quotesApi.getQuotes,      storeGetQuotes)
  const { data: requests } = useApiData(requestsApi.getRequests,  storeGetRequests)
  ...
  const adminStats = [
    ...
    {
      label: 'New Requests', value: String(openRequests.length), change: urgentReqs > 0 ? `${urgentReqs} urgent` : 'None urgent',
                ...
                {recentJobs.map((job) => {
                  ...
                  return (
                    <tr key={job.id} style={styles.tr}>
                      ...
                      <td style={styles.td}>{job.techName}</td>
                      <td style={styles.td}>
              ...
              {todayJobs.map((job) => (
                <div key={job.id} style={styles.scheduleItem}>
                  ...
                  <div style={styles.scheduleBody}>
                    ...
                    <p style={styles.scheduleMeta}>{job.type} · {job.techName}</p>
