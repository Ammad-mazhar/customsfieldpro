...
export default function Quotes() {
  ...
  async function submitCreate() {
    ...
    let c = clients.find(x=>x.id===form.clientId)
    if (form.clientId === '__new__') {
      if (!newClientForm.firstName.trim()) { setErrs(p=>({...p,clientId:'First name required'})); return }
      try {
    ...
    const n={
      ...
      clientId:resolvedClientId,clientName:c ? clientDisplayName(c) : '',clientPhone:c?.phone||'',clientEmail:c?.email||'',
          ...
          {tab==='all'&&(
            <div>
              <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
                <FilterDropdown
                  groups={[
                    { label: 'Status', key: 'statusF', options: ['All','Sent','Approved','Declined','Draft'].map(s => ({ value: s })) },
                  ]}
                  values={{ statusF }}
                  onChange={(key, val) => setStatusF(val)}
                  onReset={() => setStatusF('All')}
                />
                    ...
                    {visible.map(q=>(
                      <tr key={q.id} onClick={()=>open(q.id)} style={{borderBottom:'1px solid #f8f9fa',cursor:'pointer'}}
                        ...
                        <td style={{padding:'11px 14px',fontWeight:600,color:'#1a1d23',fontSize:13.5,whiteSpace:'nowrap'}}>
                          {q.clientName}
                          {q.clientId && <span style={{marginLeft:6,fontFamily:'monospace',fontSize:11,background:'#f0f9ff',color:'#0369a1',border:'1px solid #bae6fd',borderRadius:4,padding:'1px 5px'}}>{q.clientId}</span>}
                        </td>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:20}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                ...
                <div style={C}>
                  ...
                  {[['Quote #',sel.id],['Created',sel.created],['Valid Until',sel.expires],['Status',<Bdg key="s" label={sel.status}/>],...(sel.claimNumber?[['Claim #',<span key="cn" style={{fontFamily:'monospace',fontSize:13,color:'#854d0e',background:'#fefce8',border:'1px solid #fde68a',borderRadius:4,padding:'1px 6px'}}>{sel.claimNumber}</span>]]:[])] .map(([l,v])=>(
              ...
              {(sel.linkedJobNumber||sel.linkedInvoiceNumber)&&(
                <div style={C}>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:10}}>
                    {sel.linkedJobNumber&&(
                      <div style={{display:'flex',alignItems:'center',gap:8,background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:8,padding:'8px 14px'}}>
                        ...
                        <span style={{fontFamily:'monospace',fontSize:13,color:'#15803d',fontWeight:600}}>{sel.linkedJobNumber}</span>
                      </div>
                    )}
                    {sel.linkedInvoiceNumber&&(
                      <div style={{display:'flex',alignItems:'center',gap:8,background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:8,padding:'8px 14px'}}>
                        ...
                        <span style={{fontFamily:'monospace',fontSize:13,color:'#1e40af',fontWeight:600}}>{sel.linkedInvoiceNumber}</span>
                      </div>
                    )}
                  </div>
                </div>
