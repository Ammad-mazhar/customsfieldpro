...
import {
  getInvoices as storeGetInvoices, saveInvoices, saveInvoice,
  getClients as storeGetClients,
  getJobs as storeGetJobs,
  getSettings,
} from '../data/store'
...
export default function Invoices() {
  ...
  const { data: invoices, setData: setInvoices, loading: invLoading, error: invError } = useApiData(invoicesApi.getInvoices, storeGetInvoices)
  const { data: clients } = useApiData(clientsApi.getClients, storeGetClients)
  const { data: jobs }    = useApiData(jobsApi.getJobs, storeGetJobs)
  ...
  function updateStatus(id, status) {
    ...
    invoicesApi.updateInvoice(id, { status }).catch(() => {})
  ...
  const clientJobs = jobs.filter(j=>!form.clientId||j.clientId===Number(form.clientId))
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      {invLoading&&<div style={{fontSize:12.5,color:'#9ca3af',display:'flex',alignItems:'center',gap:6}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{animation:'spin 0.8s linear infinite'}}><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>Syncing with server…</div>}
      {invError&&<div style={{background:'#fef2f2',border:'1px solid #fecaca',color:'#dc2626',borderRadius:8,padding:'8px 14px',fontSize:13}}>API unavailable — showing cached data</div>}
            ...
            <div>
              ...
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead><tr>{['Invoice #','Client','Job Ref','Issued','Due Date','Amount','Status','Actions'].map(h=>(
                  ...
                  <tbody>
                    {visible.map(inv=>(
                      <tr key={inv.id} onClick={()=>open(inv.id)} style={{borderBottom:'1px solid #f8f9fa',cursor:'pointer'}}
                        ...
                        <td style={{padding:'11px 14px',fontWeight:600,color:'#1a1d23',fontSize:13.5,whiteSpace:'nowrap'}}>{inv.clientName}</td>
                        <td style={{padding:'11px 14px',fontFamily:'monospace',fontSize:12,color:'#9ca3af'}}>{inv.jobRef||'—'}</td>
                    ...
                    {!visible.length&&<tr><td colSpan={8}><div style={{textAlign:'center',padding:'48px 24px'}}>
