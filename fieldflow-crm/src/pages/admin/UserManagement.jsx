...
function EditModal({ user: target, currentUserId, onSave, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    colorLight:      target.colorLight ?? '#EFF6FF',
    colorName:       target.colorName  ?? 'Ocean Blue',
  ...
  function handleSave() {
    ...
    const updated = {
      ...
      colorLight: needsColor ? form.colorLight : undefined,
      colorName:  needsColor ? form.colorName : undefined,
      status:     form.status,
          ...
          {needsColor && (
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                {TECH_COLORS.map(c => {
                  const isSelected = form.color?.toLowerCase() === c.hex.toLowerCase()
                  return (
                    <button type="button" key={c.hex} title={c.name}
                      onClick={() => setForm(p => ({ ...p, color: c.hex, colorLight: c.light, colorName: c.name }))}
                      style={{ width: 28, height: 28, borderRadius: '50%', background: c.hex, border: 'none', cursor: 'pointer', outline: isSelected ? `3px solid ${c.hex}` : '2px solid transparent', outlineOffset: 2, flexShrink: 0, position: 'relative' }}>
                      {isSelected && (
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}>
              ...
              {form.colorName && <p style={{ fontSize: 11.5, color: '#6b7280', margin: 0 }}>{form.colorName}</p>}
            </div>
          )}
...
const BLANK = {
  ...
  role: 'staff', phone: '', specialty: '',
  color: '#2563EB', colorLight: '#EFF6FF', colorName: 'Ocean Blue',
  status: 'active',
}
    ...
    <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 560 }}>
      ...
      {needsColor && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
            {TECH_COLORS.map(c => {
              ...
              return (
                <button type="button" key={c.hex} title={c.name}
                  onClick={() => setForm(p => ({ ...p, color: c.hex, colorLight: c.light, colorName: c.name }))}
                  style={{ width: 30, height: 30, borderRadius: '50%', background: c.hex, border: 'none', cursor: 'pointer', outline: isSelected ? `3px solid ${c.hex}` : '2px solid transparent', outlineOffset: 2, flexShrink: 0, position: 'relative' }}>
                  {isSelected && (
          ...
          {form.colorName && <p style={{ fontSize: 11.5, color: '#6b7280', margin: 0 }}>{form.colorName} — appears on Scheduler and map pins</p>}
        </div>
      )}
...
export default function UserManagement() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        ...
                        <tr key={u.id}
                          ...
                          <td style={{ padding: '12px 14px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <div style={{
                                ...
                                background: u.role === 'technician' && u.color ? u.color : avatarColor(u.role),
                          ...
                          <td style={{ padding: '12px 14px' }}>
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              ...
                              <button onClick={() => setNotesTarget(u)}
                                style={{ height: 30, padding: '0 12px', background: '#fef9c3', color: '#92400e', border: '1px solid #fde68a', borderRadius: 6, fontSize: 12.5, fontWeight: 500, cursor: 'pointer' }}>
                                📋 Notes
                              </button>
      ...
      {/* Tech/Staff notes slide-out */}
      {notesTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.25)', zIndex: 2000, display: 'flex', justifyContent: 'flex-end' }}
          onClick={e => e.target === e.currentTarget && setNotesTarget(null)}>
          <div style={{ width: 500, maxWidth: '95vw', background: '#fff', boxShadow: '-4px 0 24px rgba(0,0,0,0.14)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0f1f3', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fafafa' }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#1a1d23' }}>{notesTarget.name}</div>
                <div style={{ fontSize: 12.5, color: '#9ca3af', marginTop: 2 }}>{notesTarget.role} · {notesTarget.email}</div>
              </div>
              <button onClick={() => setNotesTarget(null)} style={{ width: 30, height: 30, borderRadius: 7, border: '1px solid #e8e9ec', background: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>×</button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
              <StaffNotes
                techId={notesTarget.id}
                techName={notesTarget.name}
                isAdmin={true}
              />
            </div>
          </div>
